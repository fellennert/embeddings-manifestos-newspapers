...
###
# scrape before 06-2007
###

scrape_it_before0607 <- function(url){
  page <- read_html(url)
  date <- page %>% 
    html_elements(".seance") %>% 
    html_text2() %>% 
  ...
  presidence <- page %>% 
    ...
    .[str_detect(., regex("P.?ésidence de M.|Président d\\’âge|P.?esidence de M.", 
                          ignore_case = TRUE))] %>% 
    str_remove_all(regex("P.?ésidence de M. |P.?ésidence de Mme|Président d\\’âge|P.?esidence de M.|P.?esidence de Mme", 
                         ignore_case = TRUE)) %>% 
    str_squish() %>% 
    str_remove("(?<=\\n).+$") %>% 
    str_remove("\\,$") %>% 
...
scrape_it_earliest <- function(url){
  page <- read_html(url)
...
scrape_function <- function(url){
  ...
  if (between(date, ymd("2007-07-23"), ymd("2013-05-29"))) return(scrape_it_before0613(url) %>% 
                                                                    rowid_to_column("id"))
  if (between(date, ymd("2006-02-10"), ymd("2007-07-23"))) return(scrape_it_before0607(url) %>% 
                                                                    rowid_to_column("id"))
  if (date < ymd("2006-02-10")) return(scrape_it_early(url) %>% 
                                         rowid_to_column("id"))
}
...
t <- scrape_function(url)


date %>% str_extract("[1-3][0-9].{1,15} 20[0-2][0-9]$|[0-9].{1,15} 20[0-2][0-9].?$") %>% 
  str_remove("(?<=^[1-9])[a-z]*") %>% 
  str_replace("(?<=^[1-9]) er ", " ") %>% 
  parse_date(format = "%d %B %Y", locale = locale("fr")) %>% 
  unique() %>% 
